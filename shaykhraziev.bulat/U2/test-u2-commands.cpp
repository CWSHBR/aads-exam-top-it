...
BOOST_AUTO_TEST_CASE(less_filters_meetings)
{
  ...
  output.close();
  BOOST_TEST(readTextFile(filename) == "32 11\n31 10\n");
...
BOOST_AUTO_TEST_CASE(less_greater_acceptance_format)
{
  ...
  output.close();
  BOOST_TEST(readTextFile(filename) == "33 9\n28 80\n32 20\n33 10\n");
...
BOOST_AUTO_TEST_CASE(out_persons_after_desc_update_acceptance_scenario)
{
  ...
  shaykhraziev::addPerson(storage.persons,
      storage.personsById,
      shaykhraziev::Person{ 100, "Sneaky person" });
  shaykhraziev::addPerson(storage.persons,
      storage.personsById,
  ...
  BOOST_TEST(shaykhraziev::executeOutPersons(storage,
      ...
      stdoutOutput));
  stdoutOutput.close();
  BOOST_TEST(readTextFile(stdoutName).empty());
  BOOST_TEST(readTextFile(filename)
      == "100 Sneaky person\n31 Mr. Bond\n100 Sneaky person\n31 Mr. Bond\n33 Agent 007\n");
